...
func (sc *SourceController) SendLog(w http.ResponseWriter, r *http.Request) {
	...
	profileKV := strings.Split(s[1], ":")
	logs.DebugLogger.Printf("profile id passed: %s", profileKV[1])
	profileId := profileKV[1]

	typeKV := strings.Split(s[2], ":")
	...
	userTopic := fmt.Sprintf("user_topic_%s", profileId)

	rh, err := myKafka.NewRecordHandler([]string{sourceTopic, userTopic})
	if err != nil {
	...
	var record models.DockerVectorDefaultLogs
	// var record avro
	...
	for _, topic := range rh.Topics {
		payload, err := rh.Ser.Serialize(topic, &record)
		if err != nil {
