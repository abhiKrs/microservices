...
import scala.jdk.CollectionConverters.{ asScalaSetConverter, mapAsJavaMapConverter, mapAsScalaMapConverter }
...
object Application {
  ...
  def main(args: Array[String]): Unit = {
    ...
    "vector-docker-logs"
    "vector-demo-logs"
    val ts = com.google.protobuf.timestamp.Timestamp
      .fromJavaProto(com.google.protobuf.util.Timestamps.fromMillis(1678383025968L))
    ...
    env.execute()
//    getEarliestTimestampForTopic("vector-docker-logs", brokerAddress, 1678383025968L)
  }

  private def offsetsNotAvailableForTopic(topic: String, brokerAddress: String, timeStamp: Long): Boolean =
    Try {
      val kafkaConsumerProps = new Properties()
      kafkaConsumerProps.put("bootstrap.servers", brokerAddress)
      kafkaConsumerProps.put("group.id", "checkOffsetForTimestamp")
      kafkaConsumerProps.setProperty(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG,
                                     classOf[StringDeserializer].getName)
      kafkaConsumerProps.setProperty(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, classOf[StringDeserializer].getName)
      ...
      val timestampsToSearch: java.util.Map[TopicPartition, java.lang.Long] =
        consumer.assignment().asScala.map(tp => (tp, java.lang.Long.valueOf(timeStamp))).toMap.asJava

      val value = consumer.offsetsForTimes(timestampsToSearch).asScala.exists(m => m._2 == null)
      consumer.close()
      value
    } match {
...
//      TODO don't use earliest offset
        ...
        case Some(dateTimeFilter) =>
          dateTimeFilter.startTimeStamp match {
            case Some(startTimeStamp) =>
              val timestamp = com.google.protobuf.util.Timestamps
                .toMillis(com.google.protobuf.timestamp.Timestamp.toJavaProto(startTimeStamp))
              if (offsetsNotAvailableForTopic(topic, brokerAddress, timestamp)) {
          ...
          dateTimeFilter.endTimeStamp match {
            case Some(startTimeStamp) =>
              ...
              if (offsetsNotAvailableForTopic(topic, brokerAddress, timestamp)) {
                ...
                val stoppingOffsetsInitializer = OffsetsInitializer.timestamp(timestamp)
...
//      TODO advance watermark when no new records
      ...
      val withSearchFilter =
        if (filterRequest.searchQueries.nonEmpty && fieldNames
              .contains(Configs.MESSAGE_FIELD) && Array(LogicalTypeRoot.CHAR, LogicalTypeRoot.VARCHAR)
